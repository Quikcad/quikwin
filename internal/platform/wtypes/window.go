...
// Window is the cross-platform window surface every backend implements.
type Window interface {
	...
	// Lifecycle
	ShouldClose() bool
	// PollEvents processes everything already queued and returns immediately.
	PollEvents()
	// WaitEvents blocks until an event arrives or Post is called, then
	// processes everything queued.
	WaitEvents()
	// WaitEventsTimeout blocks until an event arrives, Post is called, or d
	// elapses, whichever comes first, then processes everything queued. A d of
	// zero or less is PollEvents.
	WaitEventsTimeout(d time.Duration)
	// Post wakes a blocked WaitEvents or WaitEventsTimeout. Safe to call from
	// any goroutine, so work finishing off the UI goroutine can ask for a
	// frame without the loop having to poll to find out.
	Post()
